...
export const ForgetPasswordPage = () => {
  ...
  const handleForget = async (event: FormEvent<HTMLFormEvent>) => {
        ...
        <div className="w-full max-w-[24rem] flex flex-col items-center">
            ...
            { !isSubmitted && (
              <form className="w-full flex flex-col bg-surface-container rounded-xl shadow-md p-md gap-sm relative overflow-hidden" 
                  ...
                  <button type="submit" 
                      disabled={ isInvalidEmail || email.length === 0 || isSubmitting }
                      className={`mt-md 
                          w-full
                          flex
                          items-center
                          ...
                          ${ isInvalidEmail || email.length === 0  || isSubmitting ? 'disabled:opacity-50' : '' }`}>
